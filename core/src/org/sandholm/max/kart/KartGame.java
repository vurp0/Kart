...
public class KartGame extends Game {
    ...
    LowResMainMenuScreen lowResMainMenuScreen;
    ...
    public void create() {
        ...
        //if (!videoPrefs.contains("fullscreen")) {
            videoPrefs.putBoolean("fullscreen", false);
        //}
        ...
        lowResMainMenuScreen = new LowResMainMenuScreen(this);
        multiplexer.addProcessor(lowResMainMenuScreen);
        Controllers.addListener(lowResMainMenuScreen);
        setScreen(lowResMainMenuScreen);
    ...
    public void transitionTo(Flow flow) {
        switch (flow) {
            ...
            case MAIN_MENU_SCREEN:
                ...
                Tween.to(getScreen(), UIScreenAccessor.FULL_SCREEN_DARKNESS, FADEOUT_TIME).target(0f).ease(TweenEquations.easeInCubic).start(tweenManager).setCallback(new SwitchScreenCallback(mainMenuScreen)).setCallbackTriggers(TweenCallback.COMPLETE);
                break;
            case SETTINGS_SCREEN:
                MenuScreen settingsScreen = platformSpecifics.newSettingsScreen(this);
                Tween.to(getScreen(), UIScreenAccessor.FULL_SCREEN_DARKNESS, FADEOUT_TIME).target(0f).ease(TweenEquations.easeInCubic).start(tweenManager).setCallback(new SwitchScreenCallback(settingsScreen)).setCallbackTriggers(TweenCallback.COMPLETE);
                break;
            case CONTROLLER_MAPPING_SCREEN:
                ControllerMappingScreen controllerMappingScreen = new ControllerMappingScreen(this);
                Tween.to(getScreen(), UIScreenAccessor.FULL_SCREEN_DARKNESS, FADEOUT_TIME).target(0f).ease(TweenEquations.easeInCubic).start(tweenManager).setCallback(new SwitchScreenCallback(controllerMappingScreen)).setCallbackTriggers(TweenCallback.COMPLETE);
                break;
            case KART_SELECT_SCREEN:
                MenuScreen kartSelectScreen = new KartSelectScreen(this);
                Tween.to(getScreen(), UIScreenAccessor.FULL_SCREEN_DARKNESS, FADEOUT_TIME).target(0f).ease(TweenEquations.easeInCubic).start(tweenManager).setCallback(new SwitchScreenCallback(kartSelectScreen)).setCallbackTriggers(TweenCallback.COMPLETE);
                break;
            ...
            case GAME_SCREEN:
                ...
                Tween.to(getScreen(), UIScreenAccessor.FULL_SCREEN_DARKNESS, FADEOUT_TIME).target(0f).ease(TweenEquations.easeInCubic).start(tweenManager).setCallback(new SwitchScreenCallback(kartGameScreen)).setCallbackTriggers(TweenCallback.COMPLETE);
                break;
            case RESULTS_SCREEN:
    ...
    class SwitchScreenCallback implements TweenCallback {
        ...
        SwitchScreenCallback(Screen screen) {
            this.screen = screen;
        }
        ...
        public void onEvent(int eventType, BaseTween<?> baseTween) {
            if (eventType == TweenCallback.COMPLETE) {
                getScreen().dispose();
                setScreen(screen);
                kartGameController.setUIController((UIController)screen);
            }
        }
    }
